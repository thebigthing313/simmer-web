...
import {
	...
	ZodGroupsRow,
	ZodGroupsUpdateToDb,
} from '@/simmerbase/schemas/groups';
...
export const groups = createCollection(
	queryCollectionOptions({
		...
		queryFn: async () => {
			...
			const parsed = data.map((item) => ZodGroupsRow.parse(item));
			return parsed;
		},
		queryClient,
		schema: ZodGroupsRow,
		...
		onInsert: async ({ transaction, collection }) => {
			...
			const parsedLocalNewItems = localNewItems.map((item) =>
				ZodGroupsInsertToDb.parse(item),
			);
			...
			const parsedServerNewItems = data.map((item) => ZodGroupsRow.parse(item));
		...
		onUpdate: async ({ transaction, collection }) => {
			...
			const parsedLocalChanges = ZodGroupsUpdateToDb.parse(localChanges);
