...
} from '@/simmerbase/schemas/group_profiles';

const { queryClient, supabase } = dataProviders;

const table = 'group_profiles';

export const group_profiles = createCollection(
	queryCollectionOptions({
		...
		queryFn: async () => {
			...
			const parsed = data.map((item) => ZodGroupProfilesRow.parse(item));
			return parsed;
		},
		queryClient,
		schema: ZodGroupProfilesRow,
		getKey: (item) => item.id,
		staleTime: Infinity,
		onInsert: async ({ transaction, collection }) => {
			const localNewItems = transaction.mutations.map((m) => m.modified);
			const parsedLocalNewItems = localNewItems.map((item) =>
				ZodGroupProfilesInsertToDb.parse(item),
			);
			...
			const parsedServerNewItems = data.map((item) =>
				ZodGroupProfilesRow.parse(item),
			);
		...
		onUpdate: async ({ transaction, collection }) => {
			...
			const parsedLocalChanges = ZodGroupProfilesUpdateToDb.parse(localChanges);
