create or replace function public.soft_delete_record (p_record_id uuid, p_table_name text) returns void language plpgsql
set
    ...
    begin
        ...
        execute format(
            'update public.%I set deleted_at = now(), deleted_by = (select auth.uid()) where id = $1',
            p_table_name
        ) using p_record_id;
    end;
$$;